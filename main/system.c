...
void SYSTEM_init_system(GlobalState * GLOBAL_STATE)
{
    ...
    // Initialize dual pool mode settings
    module->pool_mode = nvs_config_get_u16(NVS_CONFIG_POOL_MODE);
    module->pool_balance = nvs_config_get_u16(NVS_CONFIG_POOL_BALANCE);

    // Initialize dual pool statistics
    for (int i = 0; i < 2; i++) {
        module->dual_pool_shares_accepted[i] = 0;
        module->dual_pool_shares_rejected[i] = 0;
        module->dual_pool_best_diff[i] = 0;
    }

    ESP_LOGI(TAG, "Pool mode: %s, Balance: %d%%/%d%%",
             module->pool_mode == 1 ? "Dual Pool" : "Failover",
             module->pool_balance, 100 - module->pool_balance);
