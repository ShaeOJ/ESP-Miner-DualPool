...
type PoolLabel = 'Primary' | 'Fallback' | 'Dual';
type MessageType =
  | 'DEVICE_OVERHEAT'
  | 'POWER_FAULT'
  | 'FREQUENCY_LOW'
...
// Pool mode constants
const POOL_MODE_FAILOVER = 0;
const POOL_MODE_DUAL = 1;
...
export class HomeComponent implements OnInit, OnDestroy {
  ...
  public matrixActive: boolean = false;
  public matrixFading: boolean = false;
  public matrixColumns: { left: number; chars: string[]; speed: number; color: string }[] = [];
  private lastShareCount: number = 0;
  private matrixTimeout: any;
  ...
  // Dual Pool Mode Helper Methods
  public isDualPoolMode(info: ISystemInfo): boolean {
    return info.poolMode === POOL_MODE_DUAL;
  }

  public getPoolModeLabel(info: ISystemInfo): string {
    return info.poolMode === POOL_MODE_DUAL ? 'Dual Pool' : 'Failover';
  }

  public getDualPoolPrimaryPercentage(info: ISystemInfo): number {
    return info.poolBalance ?? 50;
  }

  public getDualPoolSecondaryPercentage(info: ISystemInfo): number {
    return 100 - (info.poolBalance ?? 50);
  }

  public getDualPoolTotalAccepted(info: ISystemInfo): number {
    if (!info.dualPoolStats) return info.sharesAccepted;
    return info.dualPoolStats.primaryAccepted + info.dualPoolStats.secondaryAccepted;
  }

  public getDualPoolTotalRejected(info: ISystemInfo): number {
    if (!info.dualPoolStats) return info.sharesRejected;
    return info.dualPoolStats.primaryRejected + info.dualPoolStats.secondaryRejected;
  }

  public getSecondaryPoolConnectionStatus(info: ISystemInfo): string {
    if (!this.isDualPoolMode(info)) return '';
    return info.secondaryPoolConnected ? 'Connected' : 'Disconnected';
  }

  // Calculate primary pool hashrate based on pool balance
  public getPrimaryHashrate(info: ISystemInfo): number {
    const balance = info.poolBalance ?? 50;
    return info.hashRate * (balance / 100);
  }

  // Calculate secondary pool hashrate based on pool balance
  public getSecondaryHashrate(info: ISystemInfo): number {
    const balance = info.poolBalance ?? 50;
    return info.hashRate * ((100 - balance) / 100);
  }

  // Matrix Hash Animation Methods
  private checkForNewShares(info: ISystemInfo): void {
    const currentShareCount = this.isDualPoolMode(info)
      ? this.getDualPoolTotalAccepted(info)
      : info.sharesAccepted;

    // Skip on first load (when lastShareCount is 0)
    if (this.lastShareCount > 0 && currentShareCount > this.lastShareCount) {
      this.triggerMatrixAnimation(info);
    }

    this.lastShareCount = currentShareCount;
  }

  private triggerMatrixAnimation(info: ISystemInfo): void {
    // Clear any existing timeout
    if (this.matrixTimeout) {
      clearTimeout(this.matrixTimeout);
    }

    // Generate random hash-like columns
    this.matrixColumns = this.generateMatrixColumns(info);
    this.matrixFading = false;
    this.matrixActive = true;

    // Start fading after 1.5 seconds
    this.matrixTimeout = setTimeout(() => {
      this.matrixFading = true;
  ...
  private generateMatrixColumns(info: ISystemInfo): { left: number; chars: string[]; speed: number; color: string }[] {
    const hexChars = '0123456789abcdef';
    const columns: { left: number; chars: string[]; speed: number; color: string }[] = [];
    const columnCount = Math.floor(Math.random() * 8) + 8; // 8-15 columns

    // Get theme color
    const primaryColor = getComputedStyle(document.documentElement).getPropertyValue('--primary-color').trim();
    const isLightMode = document.body.classList.contains('layout-theme-light');
    const color = isLightMode ? '#2d5a3d' : primaryColor; // Darker green for light mode

    for (let i = 0; i < columnCount; i++) {
      const charCount = Math.floor(Math.random() * 6) + 4; // 4-9 chars per column
      const chars: string[] = [];
      ...
      columns.push({
        ...
        chars,
        speed: Math.floor(Math.random() * 4) + 1, // Speed 1-4
        color
      });
    }
