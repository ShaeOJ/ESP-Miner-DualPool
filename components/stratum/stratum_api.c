...
static RequestTiming request_timings_secondary[MAX_REQUEST_IDS];
...
static void init_request_timings() {
    if (!initialized) {
        for (int i = 0; i < MAX_REQUEST_IDS; i++) {
            ...
            request_timings_secondary[i].timestamp_us = 0;
            request_timings_secondary[i].tracking = false;
        }
        initialized = true;
    }
}
...
static RequestTiming* get_request_timing_secondary(int request_id) {
    if (request_id < 0) return NULL;
    int index = request_id % MAX_REQUEST_IDS;
    return &request_timings_secondary[index];
}
...
// Secondary pool timing functions (separate array to avoid conflicts)
void STRATUM_V1_stamp_tx_secondary(int request_id)
{
    init_request_timings();
    if (request_id >= 1) {
        RequestTiming *timing = get_request_timing_secondary(request_id);
        if (timing) {
            timing->timestamp_us = esp_timer_get_time();
            timing->tracking = true;
        }
...
// ============================================================================
// Multi-socket buffer context functions (for dual pool support)
// ============================================================================

StratumBufferContext *STRATUM_V1_create_buffer_context()
{
    StratumBufferContext *ctx = malloc(sizeof(StratumBufferContext));
    if (ctx == NULL) {
        ESP_LOGE(TAG, "Failed to allocate buffer context");
        return NULL;
    }
    ctx->buffer = malloc(BUFFER_SIZE);
    if (ctx->buffer == NULL) {
        ESP_LOGE(TAG, "Failed to allocate context buffer");
        free(ctx);
        return NULL;
    }
...
void STRATUM_V1_free_buffer_context(StratumBufferContext *ctx)
{
    if (ctx) {
        free(ctx->buffer);
        free(ctx);
...
static void realloc_ctx_buffer(StratumBufferContext *ctx, size_t len)
{
    size_t old = strlen(ctx->buffer);
    size_t new_size = old + len + 1;

    if (new_size < ctx->buffer_size) {
        return;
    }

    new_size = new_size + (BUFFER_SIZE - (new_size % BUFFER_SIZE));
    void *new_buf = realloc(ctx->buffer, new_size);

    if (new_buf == NULL) {
        ESP_LOGE(TAG, "realloc failed in realloc_ctx_buffer");
        return;
    }

    ctx->buffer = new_buf;
    memset(ctx->buffer + old, 0, new_size - old);
    ctx->buffer_size = new_size;
}

char *STRATUM_V1_receive_jsonrpc_line_ctx(int sockfd, StratumBufferContext *ctx)
{
    if (ctx == NULL || ctx->buffer == NULL) {
        ESP_LOGE(TAG, "Invalid buffer context");
        return NULL;
    }
    ...
    if (!strstr(ctx->buffer, "\n")) {
        do {
            ...
            if (nbytes == -1) {
                ESP_LOGI(TAG, "Error: recv ctx (errno %d: %s)", errno, strerror(errno));
                return NULL;
            }

            realloc_ctx_buffer(ctx, nbytes);
            strncat(ctx->buffer, recv_buffer, nbytes);
        } while (!strstr(ctx->buffer, "\n"));
    }

    buflen = strlen(ctx->buffer);
    tok = strtok(ctx->buffer, "\n");
    line = strdup(tok);
    int len = strlen(line);
    if (buflen > len + 1)
        memmove(ctx->buffer, ctx->buffer + len + 1, buflen - len + 1);
    else
        strcpy(ctx->buffer, "");
    return line;
}
